...
| Dimension | Levels |
|---|-----|
| `in-context learning` — how much information? | **Simple** (zero-shot, minimal) · **Standard** (zero-shot + elaborated scoring rules) · **Few-shot** (3 exemplars, one per score level) |
| `thought generation` — reason before or after? | **Rate-explain** (score first, then rationale) · **Analyze-rate** (analysis first, then score) |
